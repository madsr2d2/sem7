...
class sat_filter_scoreboard(uvm_scoreboard):
    ...
    async def run_phase(self):
        ...
        cocotb.start_soon(
            self.sample_ap_uvc(
                self.uvc_ssdt_consumer_fifo, self.uvc_ssdt_consumer_queue
            )
        )
    ...
    def check_phase(self):
        ...
        # __RELEASE_REMOVE_BEGIN__
        self.logger.info(
            f"Scoreboard found {self.succes} successes and {self.failure} failures"
        )
    ...
    def print_scb(self):
        ...
        self.logger.debug(
            f" |   {self.uvc_ssdt_consumer_item.data:4d}   |   {self.ref_model_item.data:4d}   |"
        )
        self.logger.debug(f"{'-' * 35}")
        self.logger.debug(
            f" \n \
