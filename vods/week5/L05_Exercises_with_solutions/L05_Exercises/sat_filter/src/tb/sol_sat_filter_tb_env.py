...
from sol_sat_filter_scb import sat_filter_scoreboard
...
class sat_filter_tb_env(uvm_env):
    ...
    def build_phase(self):
        ...
        self.virtual_sequencer = sat_filter_tb_virt_sequencer.create(
            "sat_filter_tb_top_seqr", self
        )
        ...
        ConfigDB().set(self, "sat_filter_tb_scb", "cfg", self.cfg)
        ...
        self.ref_model_handler = sat_filter_ref_model.create(
            "sat_filter_ref_model", self
        )
    ...
    def connect_phase(self):
        ...
        self.virtual_sequencer.ssdt_producer_sequencer = (
            self.uvc_ssdt_producer.sequencer
        )
        self.virtual_sequencer.ssdt_consumer_sequencer = (
            self.uvc_ssdt_consumer.sequencer
        )
        ...
        # __RELEASE_REMOVE_BEGIN__
        self.uvc_ssdt_consumer.analysis_port.connect(
            self.scoreboard.uvc_ssdt_consumer_fifo.analysis_export
        )
        # __RELEASE_REMOVE_END__

        # Connect producer agent analysis port with ref. model handler
        self.uvc_ssdt_producer.analysis_port.connect(
            self.ref_model_handler.analysis_export
        )
