...
class test_sat_filter_rand_n_seq(sat_filter_tb_base_test):
    """ Test that launches the 'sat_filter_rnd_itr_seq' sequence:
    - Randomize the sequence to set the number of iterations
    - Start the virtual sequence to generate the data traffic.
    """

    def __init__(self, name="test_sat_filter_sat_filter_rnd_itr_seq", parent=None):
    ...
    async def run_phase(self):
        ...
        # Randomize the minimum number of sequences
        with self.virt_sequence.randomize_with() as seq:
            seq.itr_nbr >= _MIN_SEQS_NUMBER_
