...
class sat_filter_tb_base_test(uvm_test):
    ...
    def build_phase(self):
        ...
        self.producer_assert_check = ssdt_interface_assert_check(clk_signal   = self.dut.clk,
                                                                 rst_signal   = self.dut.rst,
                                                                 valid_signal = self.dut.in_valid,
                                                                 data_signal  = self.dut.in_data)

        self.consumer_assert_check = ssdt_interface_assert_check(clk_signal   = self.dut.clk,
                                                                 rst_signal   = self.dut.rst,
        ...
        self.producer_assert_check._set_width_values(self.cfg.ssdt_prod_cfg.DATA_WIDTH)
        self.consumer_assert_check._set_width_values(self.cfg.ssdt_cons_cfg.DATA_WIDTH)
